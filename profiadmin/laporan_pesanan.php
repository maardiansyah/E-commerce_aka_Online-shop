                          ...
                          <table class="table table-striped table-bordered table-hover">
                            <thead>
                              <tr>
                                ...
                                <th width="=178"><div align="center">Supplier</div></th>
                                <th width="=178"><div align="center">Nama Customer</div></th>
                                <th width="=178"><div align="center">Tanggal Pesan</div></th>
                                <th width="=178"><div align="center">Harga</div></th>
                            ...
                            <tbody>
                              ...
                              $noPage = (isset($_GET['page']))?$_GET['page']:1;
                                ...
                                ON `tbl_customer`.`Kd_Customer` = `tbl_pesanan`.`Kd_Customer` ORDER BY `id_barang`";

                                $query = "SELECT * FROM `tbl_produk` INNER JOIN `tbl_kategori`
                                ON `tbl_produk`.`Kd_Kategori`=`tbl_kategori`.`Kd_Kategori` INNER JOIN `tbl_supplier`
                                ON `tbl_produk`.`Kd_Supplier`=`tbl_supplier`.`Kd_Supplier` INNER JOIN `tbl_pesanan`
                                ON `tbl_produk`.`Kd_Barang`=`tbl_pesanan`.`Kd_Barang` INNER JOIN `tbl_customer`
                                ON `tbl_customer`.`Kd_Customer`=`tbl_pesanan`.`Kd_Customer` ORDER BY `id_barang` LIMIT $offset, $dataPerPage";
                               ...
                               <tr>
                                  ...
                                  <td><div align="left"><?php echo $data['Tanggal_Pesan'] ?></div></td>
                                  <td><div align="left">Rp. <?php echo number_format($data['Harga_Jual'],0,',','.') ?></div></td>
                                  <td><div align="left"><img src="produk/<?php echo['Foto'] ?>" width="100"></div></td>
                                </tr>
                             <?php 
                             $No++;
                           } 
                           ...
                           <tr>
                                 <td colspan="11">
                                   <div class="center">
                                    ...
                                    //link akhir
                                    if($noPage != $jumPage)
                                      echo"<a href='".$_SERVER['PHP_SELF']."?page=".($noPage+1)."' class=active>Last&raquo</a>&nbsp&nbsp";
                                      ?>
                                     </div>
                                   </div>
                                 </td>
