                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-body">
                            ...
                            <div class="col-lg-6">
                            	<?php 
                            		$query = "SELECT * FROM tbl_user WHERE id = '".$_REQUEST['id']."' ORDER BY password";
                            		$result = mysql_query($query); //xecute the query
                            		$data = mysql_fetch_array($result);
                            	 ?>
                            	 <form role="form" action="ubah_user.php?id=<?php echo $data['id']?>" method="post">
                            	 	<div class="form-group">
                            	 		<label>Username</label>
                            	 		<input class="form-control" name="username" placeholder="enter text" value="<?php echo $data['username']?>" readonly>
                            	 	</div>
                            	 	<div class="form-group">
                            	 		<label>Password</label>
                            	 		<input class="form-control" name="password" placeholder="Enter text" value="<?php echo $data['password']?>">
                            	 	</div>
                            	 	<div class="form-group">
                            	 		<label>Nama</label>
                            	 		<input name="nama" class="form-control" placeholder="Enter text" value="<?php echo $data['nama']?>">
                            	 	</div>
                            	 	<div>
